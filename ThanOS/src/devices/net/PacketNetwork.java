...
// TODO: REENABLE AND IMPLEMENT COMMENTED LINES, UNIGNORE UNIT
...
public class PacketNetwork {
    ...
    public static void dissect(int addr, int len) {
        if (DEBUG) {
            Console.print("got packet with length ");
            Console.println(len);
        }
                ...
                if (intNH(arp.tpa) == myIP) {
                    switch (shortNH(arp.oper)) {
                        case ARP_OPER_REQUEST:
                            if (DEBUG) Console.println("arp request for us");
                        ...
                        case ARP_OPER_RESPONSE:
                            ...
                            Console.print("got arp answer for ");
                            //Console.printIP(intNH(arp.spa));
                            Console.print(": ");
                            //Console.printMAC(responseMAC);
                            Console.println();
                            if (intNH(arp.spa) == lastIP) macOfLastARP = responseMAC;
                            break;
                        default:
                            if (DEBUG) Console.println("invalid arp operation");
                    }
                } else if (DEBUG) Console.println("arp request with other IP");
                break;
            case ETH_TYPE_IP:
                IP ip = (IP) MAGIC.cast2Struct(addr);
                if (myIP == 0 || ip.dstIP == -1 || intNH(ip.dstIP) == myIP) {
                    if (DEBUG) {
                        Console.print("got ip packet proto ");
                        Console.println((int) ip.prot & 0xFF);
                    }
                    switch (ip.prot) {
                        case IP_PROT_ICMP:
                            ...
                            switch (icmp.type) {
                                case ICMP_TYPE_REQUEST:
                                    if (DEBUG) Console.println("got icmp request");
                                    answerICMPRequest(addr, len);
                                    break;
                                case ICMP_TYPE_REPLY:
                                    Console.print("got icmp reply from ");
                                    //Console.printIP(intNH(icmp.ip.srcIP));
                                    if (len == ICMP_LEN_woDATA + 4) {
                                        int time = MAGIC.rMem32(MAGIC.addr(icmp.data[0]));
                                        Console.print(", round trip time in us: ");
                                        //Console.print((int)System.getMicroSeconds()-time);
                                    }
                                    Console.println();
                                    break;
                                default:
                                    if (DEBUG) Console.println("unknown ICMP type");
                            }
                            break;
                        case IP_PROT_UDP:
                            UDP udp = (UDP) MAGIC.cast2Struct(addr);
                            if (DEBUG) {
                                Console.print("udp destination port: ");
                                Console.println((int) shortNH(udp.dstPort) & 0xFFFF);
                            }
                            if (shortNH(udp.dstPort) == DHCP_CLIENT_PORT) {
                                if (DEBUG) Console.println("got dhcp answer");
                                DHCP dhcp = (DHCP) MAGIC.cast2Struct(addr);
                                if (dhcp.xid != 0xDEADBEEF) {
                                    if (DEBUG) Console.println("not our session id");
                                } else if (dhcp.op != DHCP_RESPONSE) {
                                    if (DEBUG) Console.println("not a response");
                                } else {
                                    Console.print("offered your/client address ");
                                    //Console.printIP(myIP=intNH(dhcp.yiAddrIP)); //as we do dhcp, this should be done after following dhcp request and dhcp ack
                                    Console.println();
                                }
                            }
                            break;
                        default:
                            if (DEBUG) Console.println("unknown ip proto");
                    }
                } else if (DEBUG) {
                    Console.print("ip packet with other IP: ");
                    //Console.printIP(intNH(ip.dstIP));
                    Console.println();
                }
                break;
            default:
                if (DEBUG) Console.println("unknown eth type");
        }
    }
    ...
    public static void sendARP(boolean request, long targetMAC, int otherIP) {
        if (!checkNIC()) return;
        int packetAddr;
        byte[] buf = Network.getTempTXBuffer();
        if (buf == null) {
            Console.println(NO_FREE_TX);
            return;
        }
        ...
        macOfLastARP = 0l;
        lastIP = otherIP;
        Network.nic.send(0, 0, packetAddr, ARP_LEN);
        Console.println("arp packet sent");
    }

    public static void sendICMP(long mac, int ip) {
        if (!checkNIC()) return;
        if (lastIP == 0 || lastIP == -1 || macOfLastARP == 0l) {
            Console.println("need ip and mac address first");
            return;
        }
        byte[] buf = Network.getTempTXBuffer();
        if (buf == null) {
        ...
        icmp.chk = 0;
        //int time=(int)(System.getMicroSeconds());
        ...
        Network.nic.send(0, 0, addr, ICMP_LEN_woDATA + 4);
    }
    ...
    public static void sendDHCP() {
        ...
        //send it
        Network.nic.send(0, 0, packetAddr, DHCP_LEN);
        Console.println("dhcp packet sent");
    }
    ...
    private static void answerICMPRequest(int requestAddr, int len) {
        ...
        Network.nic.send(0, 0, answerAddr, len);
    }
    ...
    private static boolean checkNIC() {
        if (Network.nic == null) {
            if (!skipPrintNoNetworkCard) {
                ...
                Console.println("no network card found");
            }
            return false;
        }
        myMAC = Network.nic.getMacAddress();
        return true;
    }
